...
arised during this project.

TODO: overview of your results (scritto dal prof)
...
The features classification is itself a core step, which is needed for
the real bills classification. Indeed, the pipeline steps before the
features classification can be viewed as a giant image preprocessing
step, with the goal to preprare a nice subset of features to be used
in the classifier.
...
Then, we are ready to train the classifier in order to predict a bill
category. For completing this task we proposed two variants of
classifier:
\begin{itemize}
  \item the Naive Bayes classifier,
  \item the ``deterministic classifier'', based on words frequency and
    a ground truth vocabulary.
\end{itemize}

This two methods are based on different ideas. The Naive Bayes
classifier comes from the family of simple probabilistic classifiers
based on applying Bayes' theorem with strong (naive) independence
assumptions between the features. Indeed, it calculates the
probability of a certain word to be seen in a document and it uses
that probability in classification phase for predicting the document
class. For our task we need the Multinomial Naive Bayes classifier, as
we have to predict multiple classes, not only two. The deterministic
classifier instead is based on the idea that each document category
has a small subset of words that are very relevant for the
classification. Indeed, we have arbitrary defined the vocabulary that
specifies what words we expect to find in a document based on its
category. Before manually building the vocabulary, we tried to let the
machine builds automatically it. See the Experiment REF.

The implementation of all components, except the deterministic
classifier (which is implemented manually), are highly based on
scikit-learn tools\footnote{In particular, we referred to the awesome
  guide \href{
    https://scikit-learn.org/stable/tutorial/text\_analytics/working\_with\_text\_data.html}{
    ``Working with Text Data''} form Scikit Learn.}  Indeed, bag of
words and counting text occurencies are implemented with the
\codeinline{CountVectorizer} from the scikit-learn package
\codeinline{sklearn.feature\_extraction.text}. The frequency
extraction is implemented with \codeinline{TfidfTransformer} from the
same package as before, while the Naive Bayes classifier is
implemented with \codeinline{MultinomialNB} from \codeinline{from
  sklearn.naive\_bayes} package.
